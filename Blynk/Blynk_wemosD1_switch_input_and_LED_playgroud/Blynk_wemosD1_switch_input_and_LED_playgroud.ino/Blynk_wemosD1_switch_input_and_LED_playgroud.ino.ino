...
BLYNK_WRITE(V11)
{
  ...
  else if(pinValue == 0){                   //If value is 0 run this command
...
//    lastChangeTime = t;
  ...
  if (t - lastChangeTime > 50) {
    Serial.print("taso1");
      if (state != currState) {
      Serial.print("taso2");
      Serial.print("HW kytkin on 1");
      currState = state;
      lastChangeTime = t;
//        if (currState == 1){
//        }
//        else if(currState == 0){
//          Serial.print("HW kytkin on 0");
        }
        }
  Serial.println("ei kayty aikavertailussa");
//  currState = state;       
    }
  prevState = state;
}

void virtualledcontrol()
{
  if (currState == 1) {
